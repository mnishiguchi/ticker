...
require_relative "ticker/version"
require "net/http"
require "json"
...
# ## Examples
#
#     Ticker.query_prints("AAPL,MSFT,GOOG,BTC-USD")
#     # AAPL        131.78     -0.18    (-0.14%) *
#     # MSFT        231.80     -0.16    (-0.07%) *
#     # GOOG       1833.50     -2.24    (-0.12%) *
#     # BTC-USD   34415.86    478.23     (1.41%)
#
module Ticker
  ...
  class << self
    def query_print(symbols)
      puts format(query(symbols))
    end

    def query(symbols, api = QuoteApi)
  ...
  class ResultFormatter
    ...
    def self.call(symbol, price, diff, percent, non_regular_market_sign)
      format("%<symbol>-10s%<price>8.2f%<diff>10.2f    (%<percent>.2f%%) %<non_regular_market_sign>s\n",
             symbol: symbol, price: price, diff: diff, percent: percent,
